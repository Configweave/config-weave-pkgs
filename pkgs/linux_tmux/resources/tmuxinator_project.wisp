...
fn clean_name(name: string) -> Result[string, string] {
    ...
    if name.ends_with(".yml") || name.ends_with(".yaml") { Ok(name) } else { Ok(name + ".yml") }
}

fn target(params: Value) -> Result[string, string] {
    Ok(home(params) + "/.config/tmuxinator/" + clean_name(param_str(params, "name", ""))?)
}

fn check(params: Value) -> Result[CheckResult, string] {
    let p = target(params)?
    if fs::is_file(p) && fs::read(p)? == param_str(params, "content", "") { Ok(CheckResult::AlreadyConfigured) } else { Ok(CheckResult::NotConfigured) }
}
