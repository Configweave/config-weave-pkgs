...
fn plugin_line(params: Value) -> Result[string, string] {
    let plugin = param_str(params, "plugin", "")
    if plugin == "" { return Err("missing 'plugin' parameter") }
    Ok("set -g @plugin '" + plugin.replace("'", "'\\''") + "'")
}
