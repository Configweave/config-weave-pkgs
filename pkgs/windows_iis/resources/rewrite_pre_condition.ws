...
// ---------------------------------------------------------------------------
// An outbound rewrite pre-condition: the gate that keeps an outbound rule off
// responses it has no business rewriting.
//
// Written whole, like the rules, because the conditions are a child
// collection rather than an attribute.
// ---------------------------------------------------------------------------

fn pre_filter() -> string { "system.webServer/rewrite/outboundRules/preConditions" }

fn pre_name(params: Value) -> Result[string, string] {
...
fn element_filter(params: Value) -> Result[string, string] {
    Ok(pre_filter() + "/preCondition[@name='" + xp(pre_name(params)?)? + "']")
}
...
fn logical_grouping(v: string) -> Result[string, string] {
    ...
    Err("invalid 'logical_grouping' value '" + v + "'")
}
...
fn probe(params: Value) -> Result[string, string] {
    ps_out(
        ...
        ps_q(pre_filter() + "/preCondition") + " | Where-Object {{ [string]$_.name -eq " +
        ps_q(pre_name(params)?) + " }}) }} catch {{ }}; " +
        "if ($r.Count -eq 0) {{ 'ABSENT' }} else {{ $x = $r[0]; " +
        "$conds = @($x.Collection | ForEach-Object {{ " +
        "([string]$_.input + '|' + [string]$_.pattern + '|' + " +
        "([string]$_.matchType).ToLower() + '|' + ([string]$_.negate).ToLower()) }}); " +
        "[pscustomobject]@{{ logical_grouping = CwV($x.logicalGrouping); " +
        "pattern_syntax = CwV($x.patternSyntax); " +
...
// Whether the live pre-condition already matches what this step asks for.
fn converged(params: Value) -> Result[bool, string] {
    let st = probe(params)?
    if st == "ABSENT" { return Ok(false) }
    let r = json::parse(st)?
    let grouping = logical_grouping(param_str(params, "logical_grouping", ""))?
    if grouping != "" && !same(get_str(r, "logical_grouping"), grouping) { return Ok(false) }
    let syntax = pattern_syntax(param_str(params, "pattern_syntax", ""))?
    if syntax != "" && !same(get_str(r, "pattern_syntax"), syntax) { return Ok(false) }
...
fn check(params: Value) -> Result[CheckResult, string] {
    ...
    let r = json::parse(st)?
    let grouping = logical_grouping(param_str(params, "logical_grouping", ""))?
    if grouping != "" && !same(get_str(r, "logical_grouping"), grouping) {
        return Ok(CheckResult::NotConfigured)
    }
    let syntax = pattern_syntax(param_str(params, "pattern_syntax", ""))?
    if syntax != "" && !same(get_str(r, "pattern_syntax"), syntax) {
        return Ok(CheckResult::NotConfigured)
    }
...
fn apply(params: Value) -> Result[ApplyResult, string] {
    require_rewrite()?
    let name = pre_name(params)?
    let scope = scope_delegated(params)?
    let st = probe(params)?
    if !want_present(params)? {
        if st == "ABSENT" { return Ok(ApplyResult::Success) }
        log::info("removing rewrite pre-condition " + name)
        ps_run("Remove-WebConfigurationProperty" + scope + " -Filter " + ps_q(pre_filter()) +
    ...
    if st != "ABSENT" {
        script = script + "Remove-WebConfigurationProperty" + scope + " -Filter " +
            ps_q(pre_filter()) + " -Name '.' -AtElement @{{name=" + ps_q(name) + "}}; "
    }
    script = script + "Add-WebConfigurationProperty" + scope + " -Filter " + ps_q(pre_filter()) +
        " -Name '.' -Value @{{name=" + ps_q(name) + "}}; "
    let elem = element_filter(params)?
    let grouping = logical_grouping(param_str(params, "logical_grouping", ""))?
    if grouping != "" {
        script = script + "Set-WebConfigurationProperty" + scope + " -Filter " + ps_q(elem) +
            " -Name 'logicalGrouping' -Value " + ps_q(grouping) + "; "
    }
    let syntax = pattern_syntax(param_str(params, "pattern_syntax", ""))?
    if syntax != "" {
        script = script + "Set-WebConfigurationProperty" + scope + " -Filter " + ps_q(elem) +
            " -Name 'patternSyntax' -Value " + ps_q(syntax) + "; "
    }
    for spec in param_list(params, "conditions") {
        let c = parse_condition(spec)?
        script = script + "Add-WebConfigurationProperty" + scope + " -Filter " + ps_q(elem) +
            " -Name '.' -Value @{{input=" + ps_q(c.get(0).unwrap_or("")) + "; pattern=" +
            ps_q(c.get(1).unwrap_or("")) + "; matchType=" + ps_q(c.get(2).unwrap_or("")) +
            "; negate=" + (if c.get(3).unwrap_or("") == "true" { "$true" } else { "$false" }) + "}}; "
    }
    log::info("writing rewrite pre-condition " + name)
    ps_run(script)?
