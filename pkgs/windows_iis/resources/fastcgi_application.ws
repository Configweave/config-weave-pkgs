...
// ---------------------------------------------------------------------------
// One FastCGI application registration. Server-wide, and identified by the
// (fullPath, arguments) pair so one executable can host several.
// ---------------------------------------------------------------------------

fn section(params: Value) -> Result[string, string] {
    Ok("system.webServer/fastCgi")
}
...
fn attrs(params: Value) -> Result[List[List[string]], string] {
    Ok([
        ["max_instances", "maxInstances", "int"],
        ["instance_max_requests", "instanceMaxRequests", "int"],
        ["activity_timeout", "activityTimeout", "dur_sec"],
        ["request_timeout", "requestTimeout", "dur_sec"],
        ["idle_timeout", "idleTimeout", "dur_sec"],
        ["queue_length", "queueLength", "int"],
        ["protocol", "protocol", "sym"],
        ["monitor_changes_to", "monitorChangesTo", "str"],
        ["stderr_mode", "stderrMode", "sym"],
        ["signal_before_terminate_seconds", "signalBeforeTerminateSeconds", "int"],
        ["flush_named_pipe", "flushNamedPipe", "bool"],
    ])
}

// [param, symbol value, IIS token]
fn sym_map() -> List[List[string]] { [
        ["protocol", "named_pipe", "NamedPipe"],
        ["protocol", "tcp", "Tcp"],
        ["stderr_mode", "return_stderr_in_500", "ReturnStdErrIn500"],
        ["stderr_mode", "return_generic_500", "ReturnGeneric500"],
        ["stderr_mode", "ignore_and_return_200", "IgnoreAndReturn200"],
        ["stderr_mode", "terminate_process", "TerminateProcess"],
    ] }
