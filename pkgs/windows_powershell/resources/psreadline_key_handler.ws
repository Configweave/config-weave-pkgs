...
// A chord such as "Ctrl+d,Ctrl+c" has to become a marker-safe block name.
...
fn block_name(params: Value) -> Result[string, string] {
    let chord = param_str(params, "chord", "")
    if chord == "" { return Err("missing 'chord' parameter") }
    Ok("psreadline-key-" + slug(chord))
}

fn desired_body(params: Value) -> Result[string, string] {
    let chord = param_str(params, "chord", "")
    if chord == "" { return Err("missing 'chord' parameter") }
    let function = param_str(params, "function", "")
    let script_block = param_str(params, "script_block", "")
    if function == "" && script_block == "" {
        return Err("one of 'function' or 'script_block' is required")
    }
    if function != "" && script_block != "" {
        return Err("'function' and 'script_block' are mutually exclusive")
    }
    let action = if function != "" {
        " -Function " + ps_q(function)
    } else {
        " -ScriptBlock {{ " + script_block + " }}"
    }
    let described = param_str(params, "description_text", "")
    let description = if described == "" { "" } else { " -Description " + ps_q(described) }
    let mode = param_str(params, "view_mode", "unmanaged")
    let vi_mode = if mode == "unmanaged" { "" } else { " -ViMode " + pascal(mode) }
    let call = "    Set-PSReadLineKeyHandler -Chord " + ps_q(chord) + action + description + vi_mode
    Ok("if ($null -ne (Get-Command Set-PSReadLineKeyHandler -ErrorAction SilentlyContinue)) {{\n" + call + "\n}}")
}
