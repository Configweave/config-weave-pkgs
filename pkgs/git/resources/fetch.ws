...
fn section() -> string { "fetch" }

fn keys() -> List[string] {
    ["prune", "pruneTags", "parallel", "recurseSubmodules", "all", "output",
     "showForcedUpdates", "writeCommitGraph", "negotiationAlgorithm",
     "fsckObjects", "unpackLimit"]
}

fn allowed() -> Map[string, List[string]] {
    #{
        "recurseSubmodules": ["yes", "on-demand", "no"],
        "output": ["full", "compact"],
        "negotiationAlgorithm": ["consecutive", "skipping", "noop", "default"],
    }
}
