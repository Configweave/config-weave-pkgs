...
fn section() -> string { "color" }

fn keys() -> List[string] {
    ["ui", "branch", "diff", "status", "grep", "interactive", "push",
     "remote", "showBranch", "advice", "transport", "pager"]
}

// Every colour switch but color.pager (a plain boolean) takes the same set.
fn allowed() -> Map[string, List[string]] {
    let c = ["true", "false", "auto", "always", "never"]
    #{
        "ui": c, "branch": c, "diff": c, "status": c, "grep": c,
        "interactive": c, "push": c, "remote": c, "showBranch": c,
        "advice": c, "transport": c,
    }
}
