...
// It is per manager: a host may run more than one, and a refresh of scoop's
// buckets says nothing about anyone else's index.
fn stamp_path() -> string { "C:/ProgramData/config-weave/scoop-cache-updated" }
...
fn apply(params: Value) -> Result[ApplyResult, string] {
    // `scoop update` with no app name pulls the bucket repositories — the
    // manifest index — without touching any installed app.
    let out = shell::powershell("scoop update", Value::Null)?
